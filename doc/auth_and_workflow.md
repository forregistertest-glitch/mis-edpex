# ระบบยืนยันตัวตนและ Workflow อนุมัติข้อมูล
# Authentication & Data Approval Workflow

| Field | Value |
|:------|:------|
| **Doc ID** | KUVMIS-DOC-008 |
| **Version** | 1.02d |
| **Standard** | ISO 27001 / EdPEx Compliance |
| **Last Updated** | 2026-02-16T19:28:00+07:00 |
| **Author** | KUVMIS Development Team |
| **Status** | Released |

---

## 1. ภาพรวมระบบยืนยันตัวตน (Authentication)

### 1.1 Authentication Flow
- **Provider**: Google Sign-In via Firebase Auth.
- **Persistence**: `inMemoryPersistence` (User must login every time tab is refreshed or closed for security).
- **Domain Restriction**: Only emails listed in `authorized_users` collection are allowed access.

### 1.2 การควบคุมสิทธิ์ (Email Whitelist)
ไม่ใช่ทุก Google Account ที่เข้าระบบได้ — ต้องอยู่ใน **รายชื่อที่อนุญาต** ซึ่งเก็บไว้ใน Firestore collection `authorized_users`

ตัวอย่างข้อมูลใน Firestore:
```
Collection: authorized_users
Document ID: "nipon.w@ku.th"
{
  email: "nipon.w@ku.th",
  role: "admin",
  name: "นิพนธ์",
  added_at: "2026-02-10"
}
```

### 1.3 ขั้นตอนการ Login
1. User กด "Sign in with Google"
2. Google popup → เลือก account → ยืนยัน 2FA (ถ้าเปิดไว้ใน Google)
3. ระบบดึง email มาตรวจสอบกับ `authorized_users` ใน Firestore
4. ✅ พบ email → เข้าระบบได้ + กำหนด role ตามข้อมูล
5. ❌ ไม่พบ → Sign-out ทันที + แสดงข้อความ "ไม่มีสิทธิ์เข้าใช้งาน"

### 1.4 Two-Factor Authentication (2FA)
ระบบได้ 2FA ฟรีจาก Google — ถ้าผู้ใช้เปิด 2-Step Verification ใน Google Account ระบบจะบังคับยืนยันตัวตน 2 ชั้นอัตโนมัติ โดยเราไม่ต้อง implement เพิ่ม

---

## 2. ระบบ Role (บทบาท)

| Role | สิทธิ์ | ตัวอย่าง |
|------|--------|----------|
| **user** | กรอกข้อมูล, ดู Dashboard, ลบ/แก้เฉพาะข้อมูลของตัวเองที่ status = pending | staff@ku.th |
| **reviewer** | ทุกอย่างที่ user ทำได้ + Approve/Reject ข้อมูลของทุกคน + เข้าเมนู "ตรวจสอบข้อมูล" | head@ku.th |
| **admin** | ทุกอย่าง + จัดการ users + เข้า Seed page + ลบข้อมูลทุก status + เข้าเมนู "จัดการผู้ใช้" | nipon.w@ku.th |

### 2.1 Role Badge ใน Sidebar ✅
เมื่อ Login แล้ว Sidebar จะแสดง Badge ตาม Role:
- 🔴 **Admin** — badge แดง พร้อมไอคอน Shield
- 🟡 **Reviewer** — badge เหลือง พร้อมไอคอน UserCheck
- 🔵 **User** — badge น้ำเงิน พร้อมไอคอน User

### 2.2 เมนูตาม Role ✅
| เมนู | User | Reviewer | Admin |
|------|:----:|:--------:|:-----:|
| Dashboard | ✅ | ✅ | ✅ |
| หมวด KPI (Academic, Staff, Hospital, Strategic) | ✅ | ✅ | ✅ |
| กรอกข้อมูล (Input) | ✅ | ✅ | ✅ |
| ตรวจสอบข้อมูล (Review) | ❌ | ✅ | ✅ |
| จัดการผู้ใช้ (Admin) | ❌ | ❌ | ✅ |
| Reports / เอกสาร | ✅ | ✅ | ✅ |

---

## 3. Workflow อนุมัติข้อมูล (Data Approval) ✅

### 3.1 แผนภาพ Workflow

```
User กรอกข้อมูล
  → status: "pending" (รอตรวจสอบ)
  → submitted_by: email ผู้กรอก (จริง)

Reviewer/Admin ตรวจสอบ (เมนู "ตรวจสอบข้อมูล")
  → ✅ Approve → status: "approved" + reviewed_by + reviewed_at
  → ❌ Reject  → status: "rejected" + เหตุผล + reviewed_by + reviewed_at
  → ✏️ ส่งกลับแก้ไข → status: "revision_requested" + reviewed_by

User เห็นรายการที่ถูกปฏิเสธ/ส่งกลับ
  → แก้ไขแล้วส่งใหม่ → status: "pending"
```

### 3.2 สถานะข้อมูล (Entry Status)

| Status | คำอธิบาย (TH) | คำอธิบาย (EN) | สี |
|--------|---------------|---------------|-----|
| `pending` | รอตรวจสอบ | Pending Review | 🟡 เหลือง |
| `approved` | อนุมัติแล้ว | Approved | 🟢 เขียว |
| `rejected` | ปฏิเสธ | Rejected | 🔴 แดง |
| `revision_requested` | ส่งกลับแก้ไข | Revision Requested | 🟠 ส้ม |
| `deleted` | ลบแล้ว (soft delete) | Deleted | ⚫ เทา |

### 3.3 ข้อมูลที่เก็บเมื่อ Review ✅
เมื่อ Reviewer Approve/Reject จะเพิ่มฟิลด์ใน Firestore:
- `reviewed_by`: email ของ Reviewer
- `reviewed_at`: timestamp ที่ review
- `rejection_reason`: เหตุผลที่ปฏิเสธ (กรณี Reject)

---

## 4. การลบข้อมูล (Soft Delete) ✅

ระบบใช้ **Soft Delete** — ข้อมูลจะไม่ถูกลบออกจาก Firestore จริง แต่จะเปลี่ยน status เป็น `"deleted"` เพื่อให้สามารถตรวจสอบย้อนกลับได้

### 4.1 กฎการลบ

| Role | ลบข้อมูล pending ของตัวเอง | ลบข้อมูล approved | ลบข้อมูลของคนอื่น |
|------|:-:|:-:|:-:|
| **user** | ✅ | ❌ | ❌ |
| **reviewer** | ✅ | ❌ | ❌ |
| **admin** | ✅ | ✅ | ✅ |

### 4.2 ข้อมูลที่เก็บเพิ่มเมื่อลบ ✅
เมื่อ soft delete จะเพิ่มฟิลด์:
- `deleted_at`: timestamp ที่ลบ
- `deleted_by`: email ของผู้ลบ
- `previous_status`: status ก่อนลบ (เพื่อ restore ได้)

---

## 5. Admin Panel: จัดการผู้ใช้งาน ✅

เมนู "จัดการผู้ใช้" (เฉพาะ Admin) ช่วยให้จัดการ authorized_users ได้โดยไม่ต้องเข้า Firebase Console

### 5.1 ฟีเจอร์
- 📋 **ดูรายชื่อผู้ใช้** — ตาราง Email, ชื่อ, Role, วันที่เพิ่ม
- ➕ **เพิ่มผู้ใช้ใหม่** — กรอก Email, ชื่อ, เลือก Role
- 🔄 **เปลี่ยน Role** — Dropdown เปลี่ยน Role ได้ทันที
- 🗑️ **ลบผู้ใช้** — ลบออกจากระบบ (ป้องกันไม่ให้ลบตัวเอง)

### 5.2 ข้อจำกัดด้านความปลอดภัย
- ไม่สามารถลบตัวเองได้
- ไม่สามารถเปลี่ยน Role ของตัวเองได้
- ผู้ใช้ที่ถูกลบจะไม่สามารถเข้าระบบได้ทันที

### 5.3 ฟีเจอร์ความปลอดภัยขั้นสูง (Advanced Security)
- **Login Logs:** บันทึกประวัติการเข้าใช้งาน (IP, Geo Location, User Agent) เพื่อ Audit ย้อนหลัง
- **Export Policy:** การดาวน์โหลดข้อมูล (CSV) ทำได้เฉพาะ Super Admin เท่านั้น
- **Mock Data Restriction:** เครื่องมือสร้าง/ลบข้อมูลจำลอง (Mock/Clear) จะใช้งานได้เฉพาะใน Environment ที่กำหนด และจำกัดสิทธิ์สูงสุด (Super Admin Only) เพื่อป้องกัน Data Loss

---

## 6. การป้องกันหน้า Seed ✅

หน้า `/seed` เป็นเครื่องมือสำหรับ admin ใช้ Seed ข้อมูลตัวอย่างเท่านั้น

- ต้อง login ก่อนเข้าถึง
- เฉพาะ role = **admin** เท่านั้นที่เข้าได้
- ผู้ใช้ทั่วไปจะเห็นหน้า "ไม่มีสิทธิ์เข้าถึง"

> **หมายเหตุ:** หน้า Seed จะใช้งานเฉพาะช่วงเริ่มต้นระบบ หลังจาก User เริ่มกรอกข้อมูลจริงแล้ว จะยกเลิกการใช้งานหน้านี้

---

## 7. Error Boundary ✅

ระบบมี Error Boundary เพื่อรองรับกรณีที่ Firebase ล่มหรือเกิดข้อผิดพลาด:

- **error.tsx**: จับ error ในหน้าเพจ → แสดงหน้า error สวยงามแทนที่จะแสดงหน้าว่าง
- **global-error.tsx**: จับ error ระดับ layout → ป้องกันหน้าเว็บค้างทั้งหมด
- มีปุ่ม "ลองใหม่" และ "กลับหน้าหลัก"

---

## 8. โครงสร้างไฟล์ที่เกี่ยวข้อง

```
src/
├── contexts/
│   └── AuthContext.tsx              ← React Context สำหรับ Auth
├── components/
│   ├── LoginPage.tsx                ← หน้า Login
│   ├── dashboard/
│   │   ├── Sidebar.tsx              ← เมนู + Role badge + เมนูตาม Role
│   │   ├── Header.tsx               ← ชื่อ/รูป user จริง
│   │   ├── ReviewerDashboard.tsx    ← หน้า Approve/Reject (B2)
│   │   └── AdminPanel.tsx           ← หน้าจัดการผู้ใช้ (B4)
│   └── kpi-input/                   ← Sub-components
├── app/
│   ├── layout.tsx                   ← Wrap AuthProvider
│   ├── page.tsx                     ← Login gate + ทุก tabs
│   ├── error.tsx                    ← Error Boundary
│   ├── global-error.tsx             ← Global Error Boundary
│   └── seed/page.tsx                ← Auth guard (admin only)
└── lib/
    ├── firebase.ts                  ← Firebase config
    └── data-service.ts              ← CRUD + review + soft delete + admin
```

---

## 9. Firestore Collections

| Collection | คำอธิบาย |
|------------|----------|
| `kpi_master` | รายการ KPI หลัก (61 KPIs) |
| `kpi_entries` | ค่า KPI ที่กรอก (มี status, review, delete fields) |
| `authorized_users` | รายชื่อ email ที่มีสิทธิ์ + role |
| `login_logs` | บันทึกการเข้าใช้งาน (email, IP, user agent, geo location) |

---

## 10. แผนการดำเนินการ (Implementation Phases)

### เฟส A ✅ (สำเร็จ)
- [x] Rec 1: แก้ metadata
- [x] Rec 2: แยก page.tsx → 5 components
- [x] Rec 3: แยก KpiInputForm.tsx → 4 sub-components
- [x] Rec 4: Firebase Auth + Google Sign-In + Email Whitelist
- [x] Rec 5: ป้องกันหน้า Seed (admin only)
- [x] Rec 6: Error Boundary

### เฟส B ✅ (สำเร็จ)
- [x] B1: Role-based UI + แก้ submitted_by เป็น email จริง
- [x] B2: Approval Workflow (Reviewer Dashboard)
- [x] B3: Soft Delete พร้อม Audit Log
- [x] B4: Admin Panel จัดการผู้ใช้ผ่านหน้าเว็บ

---

*เอกสารนี้ปรับปรุงล่าสุดเมื่อ 12 ก.พ. 2569 — KUVMIS v1.4*
