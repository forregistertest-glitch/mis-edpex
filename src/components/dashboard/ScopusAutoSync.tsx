...
import { Loader2, Play, CheckCircle, XCircle, AlertTriangle, FileText, User, Globe } from "lucide-react";
...
export default function ScopusAutoSync({ lang }: Props) {
      ...
      try {
         ...
         for (let i = 0; i < entities.length; i++) {
            ...
            try {
               ...
               const scopusId = entity.scopus_id;
               ...
               if (!scopusId) {
                  ...
                  const searchResults = await ScopusService.searchByQuery(query, viewMode);
                  ...
                  const pubs = await ScopusService.searchByAuthorId(scopusId, viewMode);
   ...
   return (
      <div className="space-y-6">
         ...
         {/* View Mode Toggle for Auto Sync */}
         <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-slate-200">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Scopus View Mode:</span>
            ...
            {viewMode === 'COMPLETE' && (
               <span className="text-[10px] text-amber-600 font-medium">
                  {lang === 'th' ? '✨ โหมดข้อมูลครบถ้วน (แนะนำสำหรับใช้งานในเครือข่ายเข้าถึง Scopus Full Text)' : '✨ Detailed mode (Recommended for institution networks)'}
               </span>
            )}
         </div>
