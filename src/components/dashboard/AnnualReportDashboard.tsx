        ...
        const fetchAll = async () => {
            ...
            try {
                ...
                for (const master of allMasters) {
                    ...
                    const agg = await getAggregatedValue(master.kpi_id, year, master.aggregation);
                    const target = master.target_value;
                    const isMet = target !== null && agg.value !== null && agg.value >= target;

                    // Simple trend detection: compare with previous year
                    let trend: "up" | "down" | "flat" = "flat";
                    const prevEntries = await getKpiEntries(master.kpi_id, year - 1);
                    if (prevEntries.length > 0 && agg.value !== null) {
                        const prevValues = prevEntries.filter(e => e.value !== null).map(e => e.value!);
                        if (prevValues.length > 0) {
                            const prevAvg = prevValues.reduce((a, b) => a + b, 0) / prevValues.length;
                            if (agg.value > prevAvg * 1.01) trend = "up";
                            else if (agg.value < prevAvg * 0.99) trend = "down";
                        }
                    }
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="text-center">
                <h2 className="text-2xl font-bold text-slate-800 mb-1">
                    {th ? `📊 รายงานสรุปภาพรวมรายปี ${year}` : `📊 Annual Performance Report FY${year}`}
                </h2>
